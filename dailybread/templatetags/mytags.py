...
class PostNode(template.Node):
    ...
    def render(self, context):
        if self.fetch_string == 'Emergency':
            ...
            context['posts'] = Post.objects.filter(publish__lte=datetime.now()).filter(status=2).exclude(categories__slug='emergency-notification').order_by('-publish')[0:self.fetch_string]
        return ''
